...
// 	}
// }
import { positionBufferData } from '../AR/positionBufferData'; 
import { frontProjectionUVMap } from '../AR/frontProjectionUVMap';

import * as THREE from "three";
import { TRIANGULATION } from "./triangulation"; // 얼굴 메쉬 삼각형 데이터

class FacePaint {
  constructor({ textureFilePath, w, h, ref }) {
    this._renderer = new THREE.WebGLRenderer({
      antialias: true,
      alpha: true,
      canvas: ref.current,
  ...
  _addGeometry() {
    ...
    this._geometry.setIndex(TRIANGULATION);
	this._geometry.setAttribute('position', new THREE.Float32BufferAttribute(positionBufferData, 3));
    this._geometry.setAttribute('uv', new THREE.Float32BufferAttribute(frontProjectionUVMap, 2));
	this._geometry.computeVertexNormals();
  }
  ...
  _addMaterial() {
    this._textureLoader = new THREE.TextureLoader();
    const texture = this._textureLoader.load(this._textureFilePath);
    texture.encoding = undefined;//RGBAFormat
    texture.anisotropy = 16;
    this._material = new THREE.MeshPhongMaterial({
      map: texture,
      color: new THREE.Color(0xffffff),
